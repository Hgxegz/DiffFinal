...
void init_options_files(int argc, const char* argv[]) {
  ...
  while (argc-- > 0) {
    const char* arg = *argv;
    setoption(arg, "-v",       "--version",                  &showversion); //works
    setoption(arg, "-q",       "--brief",                    &showbrief); //works
    setoption(arg, "-s",       "--report-identical-files",   &report_identical); //works
    setoption(arg, "--normal", NULL,                         &diffnormal);
    setoption(arg, "-y",       "--side-by-side",             &showsidebyside); //works
    setoption(arg, "--left-column", NULL,                    &showleftcolumn);
    setoption(arg, "--suppress-common-lines", NULL,          &suppresscommon); //works
  ...
  if (showbrief && different) { printf("The files and are different.\n\n");   exit(0); }
}
